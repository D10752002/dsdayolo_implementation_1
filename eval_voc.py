...
class Evaluation(object):
    def __init__(
        ...
        self.__model = Build_Model(weight_path=self.__weight_path, resume=False, showatt=self.__showatt)
    ...
    def detection(self, p = None):
        ...
        if self.__visiual:
            ...
            bboxes_prd = self.__evalter.get_bbox(img, multi_test=False, flip_test=False, mode=self.__mode)
            if bboxes_prd.shape[0] != 0:
                boxes = bboxes_prd[..., :4]
                class_inds = bboxes_prd[..., 5].astype(np.int32)
                scores = bboxes_prd[..., 4]
                return class_inds,boxes, scores
                visualize_boxes(
                    image=img,
                    boxes=boxes,
                    labels=class_inds,
...
if __name__ == "__main__":
    initializingvar=Evaluation(gpu_id=0, weight_path=None, visiual=None, eval=False, mode=None)
    global logger
