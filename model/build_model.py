...
class Build_Model(nn.Module):
    ...
    def forward(self, x):
        out = []
        [x_s, x_m, x_l], atten, features = self.__yolov4(x)
        ...
        if self.training:
            p, p_d = list(zip(*out))
            return p, p_d, features  # smalll, medium, large
        else:
            p, p_d = list(zip(*out))
            if self.__showatt:
                return p, torch.cat(p_d, 0), atten, features
            return p, torch.cat(p_d, 0), features
